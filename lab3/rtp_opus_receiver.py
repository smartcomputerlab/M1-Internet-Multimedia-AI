...
def main():
    parser = argparse.ArgumentParser(
        description="Receive OPUS over RTP (UDP) and play it via GStreamer."
    )
    parser.add_argument("--port", type=int, default=8060, help="UDP port to listen on")
    parser.add_argument("--payload", type=int, default=97, help="RTP payload type (PT)")
    parser.add_argument("--clock-rate", type=int, default=48000, help="RTP clock rate (Hz)")
    parser.add_argument("--sink", default="autoaudiosink", help="Audio sink (e.g., autoaudiosink, alsasink)")
    args = parser.parse_args()

    Gst.init(None)

    # Caps matching the RTP Opus stream
    caps = (
